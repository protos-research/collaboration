...
class Storer(object):
    ...
    @property
    def tsfresh_format(self):
        # Use interpolation to avoid NaNs on next steps
        cleared = self.mf.interpolate(method='linear')

        # Create empty df in tsfresh compatible format
        ts_df = pd.DataFrame(columns=['id', 'time', 'kind', 'value'])

        valid_params = [p for p in self.params + self.additional_params if p != 'days_exist']
        for coin in self.coins:
            for param in valid_params:
                bulk = cleared[coin][param].loc['2014-04-01':]
                bulk = bulk.to_frame(name="value")
                bulk = pd.concat([bulk, pd.DataFrame(columns=['id', 'kind'])])
                bulk['id'] = coin
                bulk['kind'] = param
                bulk.reset_index(level=0, inplace=True)
                bulk.rename(columns={'index': 'time'}, inplace=True)
                ts_df = pd.concat([ts_df, bulk], axis=0, ignore_index=True)

        ts_df = ts_df.replace([np.inf, -np.inf], np.nan)
        ts_df = ts_df.dropna(axis='rows', how='any')
...
class Extractor(object):
    ...
    def __init__(self, coin_data: pd.DataFrame, cut_start=None, periods_of_interest=None):
        """
        Parameters cut_start and periods_of_interest are mutually exclusive
        :param coin_data: Storer.mf[coin]
        :param cut_start: Date of coin scope start in format str 'YYYY-MM-DD'
        :param periods_of_interest:
        """
        self.data = self.__crop_since_exist_start(coin_data, cut_start, periods_of_interest)
        self.cleared = self.fix_na()
        self.features = self.__features()

    @staticmethod
    def __crop_since_exist_start(coin_data, cut_start, periods_of_interest):
        try:
            assert cut_start is None or periods_of_interest is None
        except AssertionError:
            raise AssertionError("Parameters cut_start and periods_of_interest are mutually exclusive")
        if cut_start:
            return coin_data[coin_data['days_exist'] >= 1].loc[cut_start:]
        if periods_of_interest is not None:
            coin_data = coin_data.interpolate(method='linear')
            coin_data['poi'] = periods_of_interest
            coin_data = coin_data[coin_data['poi'] == True]
            del(coin_data['poi'])
            return coin_data
    ...
    def __norm(self):
        ...
        :return:
        """
        cleared = self.data.interpolate(method='linear')
        return cleared
...
def extract_features(st: Storer, start_date=None, periods_of_interest=None, coins_set=None):
    """
    ...
    for coin in coins_set:
        coin_ex = Extractor(st.mf[coin], cut_start=start_date, periods_of_interest=periods_of_interest)
...
def get_list_of_periods(periods: np.array, target_label):
    periods_list = []
    prev = np.nan
    start = None
    for idx in range(len(periods)):
        ...
        if cur != prev:
            if cur == target_label:
                ...
                periods_list.append([start, idx-1])
                start = None
        prev = cur
    return periods_list
