...
class Extractor(object):
    ...
    def bull_bear_periods(self, base_t_len_days=60, thresh=0.2, req_val=0.8) -> pd.DataFrame:
        """
        ...
        one_line = np.zeros(len(self.data.index))
        ...
        for idy in range(base_t_len_days):
            if idy == 0:
                scoring_mtrx.at[[idy]] = one_line
                continue
            scoring_mtrx.at[[idy]] = np.concatenate([one_line[idy:], one_line[:idy]])

        # fit = np.zeros(len(self.data.index))
        # for i in range(len(self.data.index)):
        #     fit[i] = np.sum(scoring_mtrx[self.data.index[i]])
