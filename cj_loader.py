...
class Storer(object):
    ...
    def prep_existance_plot(self):
        ...
        for timestamp in self.times:
            ex_df.at[timestamp] = np.count_nonzero(~np.isnan(self.mf.loc[timestamp, (slice(None), 'days_exist')])) * unit
        return ex_df
...
class Extractor(object):
    ...
    def crop_since_exist_start(self, coin_data):
        return coin_data[coin_data['days_exist']>=1]

    def na_distr(self, silent=True):
        nas = dict()
